...
时间复杂度：求前缀和O(logn), 修改某一个数O(logn)

理解：

i> 对于求前x项的和，把前x项分在不同的区间，按照x的二进制位进行分 (x去掉最后1位1，x去掉最后0位1)、(x去掉最后2位1，x去掉最后1位1)、

(x去掉最后3位1，x去掉最后2位1)...(x去掉所有0 = 0，x中只剩余一个1 (最高位1) )；

这里区间都是左开右闭 对于区间（l, r] 区间内的元素个数为 2^lowbit(r), 最多有logx个区间

c[x] 表示以x为右端点，区间内一共右lowbit个元素的区间

ii>父亲找儿子——如何计算c[x]

 x的二进制表示....010..0; 1后面一共有k个0, c[x] 中一定包含a[x], 首先x - 1, 则 x - 1为....0011...1，一共有k个1，这每个1都对应一个儿子，(这里为什么每个1对应一个儿子？ 因为运用区间划分的思想，对[x - 2 ^ k + 1, x]划分 )依次计算每个儿子（lowbit运算）每次去点最后一个1， 再加上a[x]即能算出c[x] 

iii>儿子找父亲——a[x]改变后更新c[x] 对应修改操作

父亲找儿子的逆过程，有儿子其末尾一定是0，既只有偶数才会有儿子，拆分出来的每一个儿子都是形如 ...11..10..0， 则只有把它变成...100..0就得到了父亲，
...
这里的tr[i] 相当于上面的c[i]
则树状数组的修改操作为
    for(int i = x; i <= n; i += lowbit(i)) tr[i] += c;//从下往上，每次只修改父亲
查询1~x前缀和
    int res = 0;
    for(int i = x; i; i -= lowbit(i)) res += tr[i];//最初的区间划分思想
	return res;
建树状数组通常为O(nlogn) 有O(n)建立方式先预处理前缀和，根据tr[i]的定义直接建即可
    for(int i = 1; i <= n; i ++) s[i] = s[i - 1] + a[i];
	for(int j = 1; i <= n; j ++)
		for(int i = j; i; i -= lowbit(i)) 
            tr[i] = s[i] - s[i - lowbit(i) + 1];
```

## 线段树

把区间分成线段，叶子节点只有单个元素，通常每个节点为一个s结构体，结构体中维护各种所需要的信息，用数组模拟线段树，空间要开4 * N

空间需要开4倍因为 节点编号可能达到4n这个量级，比4n要小，中间很多点没用到，实际上只用到2*n - 1个点
...
```c++
线段树对应四种操作
1、pushup 子节点更新父节点 pushdown 懒标记， 由父节点更新子节点
    单点修改不需要pushdown，区间修改需要pushdown，pushdown借鉴了query思想，当遇到一个数中区间完全包含在要更新的区间内，就在这个区间内打上标记，
    表示这个树中区间的所有子节点中的每个数都需要执行标记内容（除了这个区间本身不需要，则本身区间内的每个数已经执行过标记的内容，本身也可以写成需要的形式，即本事不执行标记内容）
    注意懒惰标记通常表示要对子节点中的元素做什么，而不是表示某一固定信息
2、build 建立线段树
3、modity 修改
    遇到要修改的区间横跨当前数中区间的左右儿子区间时，要先pushdown，因为，左右儿子区间修改完成后，要pushup，若不pushdown，子节点没有被父节点的标记修改，pushup时父节点的信息就更新错了
4、query 查询
...
void pushdown(int u)
{
    if(tr[u].add){
        ...
        left.add += tr[u].add;
        left.sum += (LL)(left.r - left.l + 1) * tr[u].add;
        right.add += tr[u].add;
        right.sum += (LL)(right.r - right.l + 1) * tr[u].add;
        tr[u].add = 0;
    }
}
...
void modity(int u, int l, int r, LL d)
{
    ...
    pushdown(u);
    int mid = tr[u].l + tr[u].r >> 1;
    if(l <= mid) modity(u << 1, l, r, d);
    if(r > mid) modity(u << 1 | 1, l, r, d);
    pushup(u);
}
...
- 势能线段树
  ...
  时间复杂度 假设每个叶子节点最多修改k次后不变， 则修改的复杂度就是O(nlogn k) 最坏情况下每个点修改k次
...
维护搜索树的性质的同时维护堆的性质，这两个性质的就可以使得树的形状确定，同时堆的性质使得树的高度不至于退化成一条链，平均树高logn
...
splay 能够O(logn)的原因是，在旋转时，对一条链的路径进行了折叠

维护数组翻转信息的splay模板

核心只有splay和rotate两个函数，其他函数，依据需要添加即可。

splay函数的作用是把x节点调整到k节点的面，k为0，即让x作为根节点。
...
朴素dijkstra : 用于单源正权值最短路，稠密图，邻接矩阵存储、
...
dijkstra的核心是：先找到距离始点最近的点（初始为始点本身），然后用此点更新与其相连的点的距离。

bellman_ford : 适用于有负权边,此算法关注点在边，可以求有边的限制的最短路

每次更新点可能会有连锁反应，一次更新了好多点，这与边的顺序有关，可能会提前得到所有点的最短路，但依赖边的顺序，因此保证解的正确要更新n次，因此最坏n全部正确更新
...
思想：先把所有边从小到大排序，然后依次枚举每条边，若该边连接的两点不连通（并查集），就把该边加入到最小生成数中
...
![cfys](.images/cfys.png)

求负环时可把所有点加入队列中（所存在超级源点可以走到所有点（且此时源点的dist必须初始化），只加入超级源点即可，把所有点加入到队列中，求负环一定是对的），求负环时，可以把队列改成栈优化，其他情况此优化不适用，
...
~~~c++
//设置哨兵0号点，f[0][j] = 0, depth[0] = 0, 因此根节点depth为1
int depth[N];
int f[N][k];//k的范围依据数据范围而定，2^k < N, 离N最进的k
...
![ssc1](.images/ssc1.png)

![scc2](.images/scc2.png)
...
void tarjan(int u)
{
	...
	for(int i = h[u]; ~i; i = ne[i]){
		...
		}else if(in_stk[ver]) low[u] = min(low[u], dfn[ver]);
	}
...
求完scc后缩点，能把原图变为有向无环图（DAG）,强连通分量的倒序就是缩完点后图的拓扑排序
...
void tarjan(int u, int fa)
{
    ...
    for(int i = h[u]; ~i; i = ne[i]){
        ...
        if(!dfn[j]){
            ...
            if(dfn[u] < low[j]) is_bridge[i] = is_bridge[i ^ 1] = true; //存储一条边两个方向，在数组中是挨着的 先偶后奇
        }else if(i != (fa ^ 1)) low[u] = min(low[u], low[j]);//若答案错误这里应改成low[u] = min(low[u], dfn[j]);
    }
...
存在欧拉路径的充分必要条件是：要么所有点的出度等于入度，要么

只有两个点出度不等于入度，一个点出度等于入度加一（起点），另一个点入度等于出度加一（终点）；
...
从 i 件物品中取，每件物品最多取1次，求在不超过容量下的最大价值

```c++
f[i][j] 表示在前i件物品中选取总容量不超过j的最大价值物品
f[i][j] = max(f[i - 1][j], f[i - 1][j - v[i]] + w[i])//第二项为i - 1 所以倒序
求解f[i][j]，可以转化为求解f[i - 1][j], f[i - 1][j - v[i]] + w[i]中的最大值，因为求解f[i]时， 只用到了f[i - 1]  因此可以把数组改成滚动，
    f[j] = max(f[j], f[j - v[i]]);
此时j要从大到小，即从V到v[i]循环，避免求解f[j]时， f[j - v[i]] 已经被更新；
    
    int n, V; //物品数目、最大容量
    int v[N], w[N];//每件物品容量、价值
	int f[N];
     for(int i = 1; i <= n; i ++)
        for(int j = 0; j <= V; j ++){
            f[i][j] = f[i - 1][j];
...
```c++
集合划分为 第i件物品取，0，1，2...K个，
状态转移方程为f[i][j] = max(f[i - 1][j], f[i - 1][j - k * v[i]] + k * w[i])
f[i][j] = max(f[i - 1][j], f[i - 1][j - v[i]] + w[i], f[i - 1][j - 2 * v[i]] + 2 * w[i], ...)
f[i][j] - v[i]] = max(f[i - 1][j - v[i]], f[i - 1][j - 2 * v[i]] + w[i], f[i - 1][j - 3 * v[i]] + 2 * w[i])
所以 f[i][j] = max(f[i - 1][j], f[i][j - v[i]] + w[i])//第二项为i所以正序
...
完全背包问题 与01背包问题代码的唯一区别就是j 不必从大到小循环

```c++
原因：这是因为要保证第i次循环中的状态f[i][v]是由状态f[i-1][j-v[i]]递推而来。换句话说，这正是为了保证每件物品只选一次，保证在考虑“选入第i件物品”这件策略时，依据的是一个绝无已经选入第i件物品的子结果f[i-1][j-v[i]]。而现在完全背包的特点恰是每种物品可选无限件，所以在考虑“加选一件第i种物品”这种策略时，却正需要一个可能已选入第i种物品的子结果f[i][j-v[i]]，所以就可以并且必须采用v=0..V的顺序循环。
```
...
假设一件物品有s件， 1 + 2 + ... 2^k < s   1 + 2 + ... 2^k  + c = s   c < 2^(k + 1)

1 , 2 + ... 2^k  , c可以组合出任意小于等于s 的数，则就把有n件物品的多重背包问题转化为有n*logs的01背包问题 
    ...
    for(int i = 1; i <= n; i ++){
        ...
        for(int r = 0; r < v; r ++){
            ...
            for(int j = r; j <= m; j += v){
                ...
                while(hh <= tt && last_dp[q[tt]] + (j - q[tt]) / v * w <= last_dp[j]) tt --;
                q[++ tt] = j;
                dp[j] = max(dp[j], last_dp[q[hh]] + (j - q[hh]) / v * w);
            }
        }
    }
...
- 背包问题求方案数
  ...
  ```c++
  1.求体积恰好装到一定容量的方案数，这种的一般都是价值等于体积，体积可以直接代表价值，因此可以确定状态从哪里转移过来。
      只需要把状态转移从求最大值换成求和即可。
  2.求最优方案的方案数，记录最大价值同时也要记录此时的方案数，因此要多开一个数组，方案数转移的时候要确定是从哪个状态转移过来的，
      把转移过来的方案数加上即可。
  ```
...
- 数位dp
  ...
  技巧：1.采用前缀和的思想，把要求的区间转化成两个区间的差

​                   2.从高位到低位，采用一颗树的形式思考，左子树是这一位选择0 ~ a[i] - 1的方案，右子树是这一位选择a[i]的方案

​						左子树所有的方案数通常可以通过预处理得到，预处理的过程大多数为dp，右子树一直能走到最后一位才能得到一种合法方案

​						（这种分类的思想是基于：最终的方案每一位都要填一个数，从高位到地位考虑，分成两个子树（这样考虑的好处是，

​							简便的使得考虑的方案都是小于区间上界的），这一位填的所有合法数的所有方案等价于所有合法的方案）。

​		数位dp通常都是写记忆化搜索

​		f[i] [j]记忆化的内容是从高到低位当前枚举到第i位，前面的状态为j(不包括第i位，j通常根据题目来定) 且不贴合上界(也就是这一位
...
```c++
   ...
   const int N = 40;
   int f[N][N];//f[i][j]表示当前搜索到第i位数字上一位是j的方案数
   int l, r;
   vector<int> nums;
   
   int dp(int cur, int pre, bool lim)
   {
   	if(cur == -1) return 1;
   
   	if(!lim && ~f[cur][pre]) return f[cur][pre];
   
   	int mx = lim ? nums[cur] : 9;
   	int res = 0;
   	for(int i = pre; i <= mx; i ++)
   		res += dp(cur - 1, i, lim && i == nums[cur]);
   	
   	return lim ? res : f[cur][pre] = res;
   }
